...
当您使用WiFi配置功能时，我们需要：  
- **收集的信息**：WiFi网络列表（包括SSID、BSSID）、设备位置信息（根据系统要求，扫描周围WiFi网络需要获取位置信息）  
- **使用目的**：用于扫描周围WiFi网络并显示可用的网络列表，以便您选择WiFi网络并输入密码进行配置  
- **使用方式**：仅在您主动使用WiFi配置功能时调用，通过系统权限弹窗明确告知您收集目的和范围，获得您的明确同意后才会进行WiFi扫描  
- **信息保护**：我们仅在配置过程中临时使用WiFi网络信息，不会存储您的WiFi密码，配置完成后不会继续收集WiFi扫描结果  

2、**蓝牙设备管理功能**：  

当您使用蓝牙设备管理功能时，我们需要：  
- **收集的信息**：蓝牙设备列表、设备位置信息（根据系统要求，扫描周围蓝牙设备需要获取位置信息）  
- **使用目的**：用于扫描、发现和管理周围的蓝牙设备（如Arduino R4/ESP32等开发板设备）  
- **使用方式**：仅在您主动使用蓝牙设备管理功能时调用，通过系统权限弹窗明确告知您收集目的和范围，获得您的明确同意后才会进行蓝牙设备扫描  
- **信息保护**：我们仅在设备管理过程中使用蓝牙设备信息，不会将这些信息用于其他目的，也不会存储蓝牙设备的敏感信息  
...
- **地理位置权限**：用于提供基于位置的服务，如附近的人、本地生活服务等，以及在扫描WiFi网络和蓝牙设备时使用（根据系统要求，扫描周围网络和设备需要获取位置信息），仅在您主动使用相关功能时调用  
- **麦克风权限**：用于语音录制、语音搜索等功能，仅在您主动使用相关功能时调用  
- **通讯录权限**：用于查找好友、推荐好友等功能，仅在您主动使用相关功能并明确同意后调用  
- **存储权限**：用于保存应用数据、缓存文件等，提升应用运行效率  
- **蓝牙权限**：用于扫描、连接和管理蓝牙设备，如Arduino R4/ESP32等开发板设备，仅在您主动使用相关功能时调用  
- **WiFi权限**：用于扫描周围WiFi网络、获取WiFi网络信息，仅在您主动使用WiFi配置功能时调用  
...
9. **WiFi配置功能**  
   当您使用WiFi配置功能时，我们需要获取您的地理位置权限（根据系统要求，扫描周围WiFi网络需要位置信息），用于扫描周围WiFi网络并显示可用的网络列表。您可以选择WiFi网络并输入密码，我们会将配置信息发送到开发板设备。我们仅在配置过程中临时使用WiFi网络信息，不会存储您的WiFi密码。  

10. **蓝牙设备管理**  
    当您使用蓝牙设备管理功能时，我们需要获取您的蓝牙权限和地理位置权限（根据系统要求，扫描周围蓝牙设备需要位置信息），用于扫描、发现和管理周围的蓝牙设备（如Arduino R4/ESP32等开发板设备）。我们仅在设备管理过程中使用蓝牙设备信息，不会将这些信息用于其他目的。  
